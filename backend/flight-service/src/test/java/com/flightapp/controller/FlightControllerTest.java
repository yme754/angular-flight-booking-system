...
class FlightControllerTest {
    ...
    void testSearchFlights() {
        ...
        searchRequest.setFrom("HYD");
        searchRequest.setTo("DEL");        
        String searchDate = "2025-12-25"; 
        searchRequest.setDate(searchDate); 
        when(flightService.searchFlights("HYD", "DEL", searchDate)).thenReturn(Flux.just(flight));
        client.post()
            .uri("/api/flight/flights/search")
            .bodyValue(searchRequest)
            .exchange()
            .expectStatus().isOk()
            .expectBody().jsonPath("$[0].airlineId").isEqualTo("A1");
        verify(flightService).searchFlights("HYD", "DEL", searchDate);
    }
